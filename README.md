<div align="center">

<!-- Typing Animation - shortened to fit properly -->
<a href="https://github.com/Adnannewb">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=1000&color=4A9EF7&center=true&vCenter=true&multiline=true&width=600&height=80&lines=Hi+there%2C+I'm+Adnan+Rahman+%F0%9F%91%8B;Full+Stack+Engineer+%7C+Python+%2F+Django+%2F+React" alt="Typing SVG" />
</a>

<br/>

<!-- Animated wave header -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0B1120,50:1E4A8A,100:4A9EF7&height=120&section=header&text=Md.+Adnan+Rahman&fontSize=40&fontColor=F0F4FF&fontAlignY=65&animation=fadeIn&desc=Full+Stack+Engineer+%7C+Python+%2F+Django+%2F+React&descAlignY=85&descSize=15&descColor=7BAFD4" width="100%"/>

</div>

---



## 🧑‍💻 About Me

```python
class AdnanRahman:
    def __init__(self):
        self.name       = "Md. Adnan Rahman"
        self.role       = "Full Stack Engineer"
        self.university = "Jashore University of Science and Technology (JUST)"
        self.degree     = "B.Sc. in Computer Science and Engineering"
        self.cgpa       = "3.50 / 4.00 (up to 6th semester)"
        self.location   = "Rangpur, Bangladesh"
        self.status     = "Open to internships & jobs"

    @property
    def stack(self):
        return {
            "backend":   ["Python", "Django", "Django REST Framework", "Node.js"],
            "frontend":  ["React", "Bootstrap 5", "Tailwind CSS"],
            "databases": ["PostgreSQL", "MySQL", "SQLite", "MongoDB"],
            "ml":        ["scikit-learn", "PyCaret", "Pandas", "NumPy"],
            "tools":     ["Git", "GitHub", "Gunicorn", "Render", "Netlify"],
        }

    def __str__(self):
        return "Building reliable, scalable web systems — one commit at a time."
```

---

## 🛠️ Tech Stack

<div align="center">

### 🐍 Languages
<table>
  <tr>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=python" width="48"/><br/>
      <sub><b>Python</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=js" width="48"/><br/>
      <sub><b>JavaScript</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=c" width="48"/><br/>
      <sub><b>C</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=cpp" width="48"/><br/>
      <sub><b>C++</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=cs" width="48"/><br/>
      <sub><b>C#</b></sub>
    </td>
  </tr>
</table>

### ⚙️ Backend
<table>
  <tr>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=django" width="48"/><br/>
      <sub><b>Django</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=python" width="48"/><br/>
      <sub><b>DRF</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=nodejs" width="48"/><br/>
      <sub><b>Node.js</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=nginx" width="48"/><br/>
      <sub><b>Gunicorn</b></sub>
    </td>
  </tr>
</table>

### 🎨 Frontend
<table>
  <tr>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=react" width="48"/><br/>
      <sub><b>React</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=bootstrap" width="48"/><br/>
      <sub><b>Bootstrap 5</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=tailwind" width="48"/><br/>
      <sub><b>Tailwind CSS</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=html" width="48"/><br/>
      <sub><b>HTML</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=css" width="48"/><br/>
      <sub><b>CSS</b></sub>
    </td>
  </tr>
</table>

### 🗄️ Databases
<table>
  <tr>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=postgres" width="48"/><br/>
      <sub><b>PostgreSQL</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=mysql" width="48"/><br/>
      <sub><b>MySQL</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=sqlite" width="48"/><br/>
      <sub><b>SQLite</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=mongodb" width="48"/><br/>
      <sub><b>MongoDB</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=redis" width="48"/><br/>
      <sub><b>MSSQL</b></sub>
    </td>
  </tr>
</table>

### 🤖 Machine Learning & Data
<table>
  <tr>
    <td align="center" width="120">
      <img src="https://skillicons.dev/icons?i=sklearn" width="48"/><br/>
      <sub><b>scikit-learn</b></sub>
    </td>
    <td align="center" width="120">
      <img src="https://skillicons.dev/icons?i=python" width="48"/><br/>
      <sub><b>PyCaret</b></sub>
    </td>
    <td align="center" width="120">
      <img src="https://skillicons.dev/icons?i=pandas" width="48"/><br/>
      <sub><b>Pandas</b></sub>
    </td>
    <td align="center" width="120">
      <img src="https://skillicons.dev/icons?i=numpy" width="48"/><br/>
      <sub><b>NumPy</b></sub>
    </td>
  </tr>
</table>

### 🔧 Tools & Deployment
<table>
  <tr>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=git" width="48"/><br/>
      <sub><b>Git</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=github" width="48"/><br/>
      <sub><b>GitHub</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=gitlab" width="48"/><br/>
      <sub><b>GitLab</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=render" width="48"/><br/>
      <sub><b>Render</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=netlify" width="48"/><br/>
      <sub><b>Netlify</b></sub>
    </td>
    <td align="center" width="100">
      <img src="https://skillicons.dev/icons?i=vscode" width="48"/><br/>
      <sub><b>VS Code</b></sub>
    </td>
  </tr>
</table>

### 🏗️ Design & Architecture
<table>
  <tr>
    <td align="center" width="130">
      <img src="https://img.shields.io/badge/OOP-0B1120?style=for-the-badge&logoColor=4A9EF7" /><br/>
      <sub><b>OOP</b></sub>
    </td>
    <td align="center" width="130">
      <img src="https://img.shields.io/badge/SOLID-0B1120?style=for-the-badge&logoColor=4A9EF7" /><br/>
      <sub><b>SOLID Principles</b></sub>
    </td>
    <td align="center" width="130">
      <img src="https://img.shields.io/badge/Design_Patterns-0B1120?style=for-the-badge&logoColor=4A9EF7" /><br/>
      <sub><b>Design Patterns</b></sub>
    </td>
    <td align="center" width="130">
      <img src="https://img.shields.io/badge/UML-0B1120?style=for-the-badge&logoColor=4A9EF7" /><br/>
      <sub><b>UML</b></sub>
    </td>
  </tr>
</table>

</div>

---

## 💡 Daily Dev Quote

<div align="center">

[![Readme Quotes](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=dark&border=true)](https://github.com/piyushsuthar/github-readme-quotes)

> ⚡ *Refreshes automatically every 24 hours*

</div>

---

## 🤝 Connect With Me

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0B1120?style=for-the-badge&logo=linkedin&logoColor=0A66C2)](https://linkedin.com/in/adnan-rahman-dev)
[![GitHub](https://img.shields.io/badge/GitHub-0B1120?style=for-the-badge&logo=github&logoColor=F0F4FF)](https://github.com/Adnannewb)
[![Email](https://img.shields.io/badge/Email-0B1120?style=for-the-badge&logo=gmail&logoColor=EA4335)](mailto:rahadnan6@gmail.com)

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:4A9EF7,50:1E4A8A,100:0B1120&height=100&section=footer&animation=fadeIn" width="100%"/>

<sub>⭐ If you find my work interesting, consider giving a star to my repos!</sub>

</div>
